const navigation = {
    ...
    social: [
      {
        name: 'Facebook',
        href: 'https://www.facebook.com/GetPrepMate/',
        icon: (props) => (
      ...
      {
        name: 'Instagram',
        href: 'https://www.instagram.com/getprepmate',
        icon: (props) => (
          <svg fill="currentColor" viewBox="0 0 24 24" {...props}>
            <path
              fillRule="evenodd"
...
export default function Footer() {
    return (
        <footer className="bg-gray-200" aria-labelledby="footer-heading">
          ...
          <div className="max-w-7xl px-6 pb-8 pt-10 sm:pt-16 lg:px-8 lg:pt-24 mx-auto">
            ...
            <div className="mt-16 border-t border-white/10 pt-8 sm:mt-20 md:flex md:items-center md:justify-between lg:mt-24">
              <div className="flex space-x-6 md:order-2">
                {navigation.social.map((item) => (
                  <a key={item.name} href={item.href} className="text-black hover:text-gray-600">
                    <span className="sr-only">{item.name}</span>
                    <item.icon className="h-6 w-6" aria-hidden="true" />
                  </a>
                ))}
              </div>
              <p className="mt-8 text-xs leading-5 text-black md:order-1 md:mt-0">
                &copy; 2023 PrepMate. All rights reserved.
              </p>
            </div>
          </div>
        </footer>
